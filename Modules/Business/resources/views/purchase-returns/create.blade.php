                    ...
                    <div class="mt-4 mb-3 order-form-section ">
                        <div class="row g-3">
                            <div class="col-lg-4">
                                ...
                                <input type="text" value="{{ $purchase->party->name ?? '' }}" class="form-control"
                                    placeholder="{{ __('Invoice no') }}." readonly>
                            </div>
                            <div class="col-lg-4">
                                <label>{{ __('Date') }}</label>
                                <input type="date" class="form-control"
                                    value="{{ formatted_date($purchase->purchaseDate, 'Y-m-d') }}" readonly>

                            </div>
                            <div class="col-lg-4">
                                <label>{{ __('Invoice No') }}.</label>
                                <input type="text" value="{{ $purchase->invoiceNumber }}" class="form-control"
                                    placeholder="{{ __('Invoice no') }}." readonly>
                            </div>
                        </div>
                    </div>
                                ...
                                <tbody class='text-start'>
                                    @foreach ($purchase->details as $detail)
                                        @php
                                            ...
                                            $original_price = $detail->productPurchasePrice;
                                            $discounted_price_per_unit = $original_price - $original_price * $discount_per_unit_factor;
